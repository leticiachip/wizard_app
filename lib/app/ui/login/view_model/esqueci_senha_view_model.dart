...
class EsqueciSenhaViewModel {
  ...
  Future<Result<bool, ExceptionApp>> _alterarSenha(
    (String, String, String) dados,
  ) async {
    final email = dados.$1;
    final senha = dados.$2;
    final codigo = dados.$3;
    Result<bool, ExceptionApp> result = await loginService.confirmarTrocaSenha(
      email,
      senha,
      codigo,
    );
  ...
  Future<Result<String, ExceptionApp>> _enviarEmailCodigo(String email) async {
    Result<String, ExceptionApp> result = await loginService
